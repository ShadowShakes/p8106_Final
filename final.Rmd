...
trainData = covid_df[indexTrain, ]
testData = covid_df[-indexTrain, ]
```
...
# feature plot of discrete variables
fea_dis = covid_df %>%
  select(-age,
         -height,
         -weight,
...
ggplot(dis_sub1, aes(category, recovery_time, fill = type)) + 
  geom_boxplot() + 
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust = 1)) + 
  facet_grid(. ~type, scales = "free_x")

ggplot(dis_sub2, aes(category, recovery_time, fill = type)) + 
  geom_boxplot() + 
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust = 1)) + 
  facet_grid(. ~type, scales = "free_x")

# correlation visualization
corrplot(cor(covid_df 
            %>% select(age,
                       height,
                       weight,
                       bmi,
...
x = dat2[indexTrain, ]
y = dat$recovery_time[indexTrain]
...
error_lm = mean((pred_lm - dat$recovery_time[-indexTrain])^2)
error_lm

# best subset model selection study
regsubsetsObj = regsubsets(recovery_time ~ ., data = trainData, method = "exhaustive", nbest = 1)
...
## Lasso Regression (not included in final report)
```{r}
# Lasso regression
set.seed(5154)
fit_lasso = train(x, y,
...
# test error
pred_lasso = predict(fit_lasso, newdata = dat2[-indexTrain, ])
error_lasso = mean((pred_lasso - dat$recovery_time[-indexTrain])^2)
error_lasso
```
